...
async function main() {
  ...
  try {
    ...
    for (const product of products) {
      ...
      if (!categoryId)
        throw new Error(`Categoria não encontrada: ${product.categoryName}`);
      ...
      for (const variant of product.variants) {
        const images = productImages[product.name]?.[variant.color] || [];

        await prisma.productVariant.create({
          data: {
            id: crypto.randomUUID(),
            name: variant.color,
            slug: generateSlug(`${product.name}-${variant.color}`),
            color: variant.color,
    ...
    console.log(
      `📊 Foram criadas ${categories.length} categorias, ${products.length} produtos com ${products.reduce((acc, p) => acc + p.variants.length, 0)} variantes.`,
    );
  } catch (error) {
